...
fun parseInput2(input: List<String>): List<List<MetalPipe>> {
    return input.mapIndexed { rowIndex, row ->
        row.mapIndexed { columnIndex, pipeChar ->
            MetalPipe(pipeChar, rowIndex, columnIndex)
        }
...
fun main() {
    ...
    fun part2(input: List<String>): Int {
        ...
        val possiblePipes = pipeMap.filter { !visitedNodes.contains(it) }

        val map2 = parseInput2(input)
        map2.forEach { pipeRow ->
            pipeRow.forEach { pipe ->
                if (visitedNodes.contains(pipe)) {
                    print(' ')
                } else {
                    print(pipe.char)
                }
    ...
    val testInput3 = readInput("Day10_tests3")
